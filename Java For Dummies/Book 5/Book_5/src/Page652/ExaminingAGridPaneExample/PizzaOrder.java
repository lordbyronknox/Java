 ...
 */
package Page652.ExaminingAGridPaneExample;
...
// Create the size pane →56
        ...
        rdoMedium.setSelected(true);                //sets 'medium' as selected by default
        ToggleGroup groupSize = new ToggleGroup();
        rdoSmall.setToggleGroup(groupSize);         //Togglegroup: allows only one member of 
        rdoMedium.setToggleGroup(groupSize);        // the group to be selected ata a time.
...
// Add the nodes to the pane →112
        ...
        grid.addRow(3, paneSize, paneCrust, paneToppings);  //VBoxs containing radial buttons added to row 3.
        grid.add(paneButtons, 2, 4);                        //HBox containing OK & Cancel added to
                                                            // to column 2 and 4.
// Set alignments and spanning →119
        grid.setHalignment(lblName, HPos.RIGHT);            //text boxes set to right alignment
        grid.setHalignment(lblPhone, HPos.RIGHT);
        grid.setHalignment(lblAddress, HPos.RIGHT);
        grid.setColumnSpan(txtName, 2);                     //textboxes SPAN set to 2.
        grid.setColumnSpan(txtPhone, 2);
        grid.setColumnSpan(txtAddress, 2);
        
// Set column widths →127
        ColumnConstraints col1 = new ColumnConstraints();   //column constraint created
        col1.setPercentWidth(33);                           //...and set to width of 33pixels.
